...
{
	MyWire->beginTransmission(DS3231_ID);
	MyWire->write(addr);       // set register address
	if (MyWire->endTransmission() != 0)
		return(0);

	MyWire->requestFrom(DS3231_ID, (int)len);
	//while (!MyWire->available()) ;   // wait
	for (uint8_t i = 0; i<len; i++) // Read x data from given address upwards...
	{
		buf[i] = MyWire->read();       // ... and store it in the buffer
	}
...
uint8_t MD_DS3231::writeDevice(uint8_t addr, uint8_t* buf, uint8_t len)
{
	...
	{
		MyWire->write(buf[i]);         // ... and send it from buffer
	}
...
MD_DS3231::MD_DS3231(TwoWire * myWire) : yyyy(0), mm(0), dd(0), h(0), m(0), s(0), dow(0),
_cbAlarm1(nullptr), _cbAlarm2(nullptr), _century(20)
{
	this->MyWire = myWire;
}
