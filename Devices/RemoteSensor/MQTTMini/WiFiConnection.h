...
void buildWebPageBuffer()
{
	...
	for (int i = 0; i < sizeof(settingItems) / sizeof(struct SettingItem); i++)
	{
		...
		case password:
			...
			break;
		case number:
			sprintf(topicBuffer, "<input name = '%s' type = 'text' value='%d'><br>",
				settingItems[i].formName, settingItems[i].value);
			break;
		case onOff:
			if (settingItems[i].value)
			{
				sprintf(topicBuffer, "<input name = '%s' type = 'text' value='on'><br>",
					settingItems[i].formName, settingItems[i].value);
			}
			else
			{
				sprintf(topicBuffer, "<input name = '%s' type = 'text' value='off'><br>",
					settingItems[i].formName, settingItems[i].value);
			}
			break;
		case yesNo:
			if (settingItems[i].value)
			{
				sprintf(topicBuffer, "<input name = '%s' type = 'text' value='yes'><br>",
					settingItems[i].formName, settingItems[i].value);
			}
			else
			{
				sprintf(topicBuffer, "<input name = '%s' type = 'text' value='no'><br>",
					settingItems[i].formName, settingItems[i].value);
			}
			break;
		}
...
void serveAdmin(ESP8266WebServer *webServer) {
	...
	if (webServer->args() == 0) {
		...
		for (int i = 0; i < sizeof(settingItems) / sizeof(struct SettingItem); i++)
		{
			...
			case number:
				int value;
				if (sscanf("%d", argValue.c_str(),&value) == 1)
				{
...
{
	Serial.printf("Starting wifi config as %s", settings.deviceNane);

	WiFi.mode(WIFI_AP);
	delay(100);

	WiFi.softAP(settings.deviceNane);
