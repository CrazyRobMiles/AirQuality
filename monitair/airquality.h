...
SoftwareSerial * airqSensorSerial;

#define AIRQ_READING_TIMEOUT_SECS 5000
#define AIRQ_ERROR_TIMED_OUT -1
#define AIRQ_ERROR_NO_SENSOR -2

#define AIRQ_SDS011_SENSOR_TYPE 1
#define AIRQ_ZPH01_SENSOR_TYPE 2
...
{
	{ AIRQ_SDS011_SENSOR_TYPE, {170,192}, 0 },
	{ AIRQ_ZPH01_SENSOR_TYPE, {0xFF,0x18}, 0 }
};
...
int getSensorType()
{
	...
	while (true)
	{
		if (ulongDiff(millis(), airqReadingStartTime) > AIRQ_READING_TIMEOUT_SECS)
		{
...
void testAirQuality()
{
	while (true)
	{
		if (updateAirqReading(&airqSensor) == SENSOR_OK)
		{
			struct airqualityReading * airqualityActiveReading =
				(airqualityReading *)airqSensor.activeReading;
			Serial.printf("Air quality reading PM10: %f PM2.5: %f\n", 
				airqualityActiveReading->pm10, airqualityActiveReading->pm25);
		}
		else
		{
			Serial.println("No reading obtained");
		}
		delay(500);
	}
}
...
void airqStatusMessage(struct sensor * airqSensor, char * buffer, int bufferLength)
{
	...
	case SENSOR_OK:
		switch (settings.airqSensorType)
		{
		case AIRQ_SDS011_SENSOR_TYPE:
			snprintf(buffer, bufferLength, "SDS011 sensor active");
			break;
		case AIRQ_ZPH01_SENSOR_TYPE:
			snprintf(buffer, bufferLength, "ZPH01 sensor active");
			break;
		default:
			snprintf(buffer, bufferLength, "Unknown sensor active");
			break;
		}
		break;
	...
	case SENSOR_OFF:
		...
		{
		case AIRQ_SDS011_SENSOR_TYPE:
			snprintf(buffer, bufferLength, "SDS011 sensor off");
			break;
		case AIRQ_ZPH01_SENSOR_TYPE:
			snprintf(buffer, bufferLength, "ZPH01 sensor off");
			break;
		default:
			snprintf(buffer, bufferLength, "Unknown sensor off");
			break;
		}
		break;
	...
	case AIRQ_ERROR_TIMED_OUT:
		...
		{
		case AIRQ_SDS011_SENSOR_TYPE:
			snprintf(buffer, bufferLength, "SDS011 sensor timed out");
			break;
		case AIRQ_ZPH01_SENSOR_TYPE:
			snprintf(buffer, bufferLength, "ZPH01 sensor timed out");
			break;
		default:
			snprintf(buffer, bufferLength, "Unknown sensor timed out");
			break;
		}
		break;
	default:
