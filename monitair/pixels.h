...
enum readingDisplayStates { veryLow, lowWarn, lowMid, lowHigh, highHigh, alert };

readingDisplayStates displayState;

readingDisplayStates getDisplayState(float value)
{
...
void setDisplayState(readingDisplayStates newState)
{
	...
	case veryLow:
		...
		break;
	case lowWarn:
		fadeWalkingColor(lamps, YELLOW_PIXEL_COLOUR, 100);
		break;
	case lowMid:
		fadeWalkingColor(lamps, BLUE_PIXEL_COLOUR, 100);
		break;
	case lowHigh:
		fadeWalkingColor(lamps, CYAN_PIXEL_COLOUR, 100);
		break;
	case highHigh:
		fadeWalkingColor(lamps, MAGENTA_PIXEL_COLOUR, 100);
		break;
	case alert:
		fadeWalkingColor(lamps, RED_PIXEL_COLOUR, 100);
	}
}

void updateReadingDisplay()
{
	struct airqualityReading * reading = (struct airqualityReading *) airqSensor.activeReading;

	readingDisplayStates newDisplayState = getDisplayState(reading->pm25);
